...
{
    esp_err_t err;
    /* Quarantaquattro e non ventiquattro. La lettura qui sotto ne chiede
       quarantaquattro, e il buffer del costruttore ne dichiarava ventiquattro:
       venti byte finivano sulla pila, sopra le variabili dichiarate subito
       dopo — fra cui `cinfo`, che e proprio quella che viene poi passata
       all'algoritmo delle dita. I tocchi arrivavano e uscivano sbagliati. */
...
// #endif
    ...
    /* Questo numero lo dice il chip, e nessuno lo aveva mai guardato prima di
       usarlo come limite di due cicli. Le sue destinazioni tengono dieci dita
       qui e MAX_FINGER_NUM piu sotto: un registro sporco — o un chip che si
       riavvia mentre lo si legge — scriveva oltre la fine di entrambe. */
    if (Finger_num > 10) Finger_num = 10;
	...
	if (Finger_num > MAX_FINGER_NUM) Finger_num = MAX_FINGER_NUM;
